...
function loadWidget(waifuPath, apiPath) {
	...
	function registerEventListener() {
		...
		$("#waifu-tool .fa-times").click(() => {
			...
			showMessage("愿你有一天能与Scat重逢。", 2000, 11);
		...
		console.log(re);
		re.toString = () => {
			showMessage("啊♂，你打开了控制台，是想要看看Scat的小秘密吗？", 6000, 9);
			return "";
		};
		$(document).on("copy", () => {
			showMessage("你都复制了些什么呀，转载要记得告诉scat哦！", 6000, 9);
		});
		$(document).on("visibilitychange", () => {
			if (!document.hidden) showMessage("哇，你终于回来了～Scat很想你欸", 6000, 9);
		});
	}
	...
	function welcomeMessage() {
		...
		if (location.href == SiteIndexUrl) { //如果是主页
			...
			else if (now > 21 && now <= 23) text = ["弟弟，熬夜会猝死的嗷", "深夜时要爱护眼睛呀！"];
			else text = "弟弟，熬夜会猝死的嗷？";
	...
	function loadRandModel() {
		...
		$.ajax({
			...
			success: function(result) {
				if (result.textures["id"] == 1 && (modelTexturesId == 1 || modelTexturesId == 0)) showMessage("我还有其他衣服呢！", 4000, 10);
